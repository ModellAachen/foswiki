...
CKEDITOR.dialog.add( 'qwikisymbol', function( editor )
{
	...
	var onClick = function( evt )
	{
		...
		if (editor.config.qwikisymbol_links[title]) {
			var link = editor.document.createElement( 'a',
				{
					attributes :
					{
						href : editor.config.qwikisymbol_links[title]
					}
				});
			link.append( img );
	...
	for ( i = 0 ; i < size ; i++ )
	{
		...
		var smileyLabelId = 'cke_smiley_label_' + i + '_' + CKEDITOR.tools.getNextNumber();
		html.push(
			'<td class="cke_dark_background cke_centered" style="vertical-align: middle;">' +
				'<a href="javascript:void(0)" role="option"',
					...
					'<img class="cke_hand" title="', config.qwikisymbol_descriptions[i], '"' +
						' cke_src="', CKEDITOR.tools.htmlEncode( config.qwikisymbol_path + images[ i ] ), '" alt="', config.qwikisymbol_descriptions[i], '"',
						' src="', CKEDITOR.tools.htmlEncode( config.qwikisymbol_path + images[ i ] ), '"',
						// IE BUG: Below is a workaround to an IE image loading bug to ensure the image sizes are correct.
						( CKEDITOR.env.ie ? ' onload="this.setAttribute(\'width\', 2); this.removeAttribute(\'width\');" ' : '' ),
					'>' +
					'<span id="' + smileyLabelId + '" class="cke_voice_label">' +config.qwikisymbol_descriptions[ i ]  + '</span>' +
				'</a>',
 			'</td>' );
	...
	return {
		title : editor.lang.qwikisymbol.title,
		minWidth : 270,
		minHeight : 120,
		contents : [
			{
