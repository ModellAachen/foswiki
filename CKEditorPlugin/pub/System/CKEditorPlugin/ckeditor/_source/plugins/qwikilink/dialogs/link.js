...
CKEDITOR.dialog.add( 'link', function( editor )
{
	...
	{
		var dialog = this.getDialog(),
			partIds = [ 'qwikiOptions', 'urlOptions', 'emailOptions' ],
	...
	var parseLink = function( editor, element )
	{
		...
		if ( !retval.type )
		{
			if ( ( anchorMatch = href.match( anchorRegex ) ) )
			{
				retval.type = 'qwiki';
				retval.qwiki = {};
				retval.qwiki.type = 'anchorOptions';
				retval.anchor = {};
			...
			else if (  href && ( urlMatch = href.match( qwikiRegex ) ) )
			{
				...
				if(urlMatch[1])
				{
					retval.qwiki.type = 'attachmenturl';
					// This type of link is generated by the file upload dialog
					if (urlMatch[1].match(/^%(?:ATTACH|PUB)URL%\//))
						urlMatch[1] = '';
					retval.qwiki.topicurl = urlMatch[1] + urlMatch[2];
				}
				else
				{
					retval.qwiki.protocol = '';
	...
	return {
								...
								[
									...
									},
									{
										...
										setup : function( data )
										{
											...
											if ( data.qwiki )
												this.setValue( data.qwiki.topicurl );
										...
										commit : function( data )
										{
											...
											data.qwiki.topicurl = this.getValue();
											this.allowOnChange = false;
										}
									}
								],
		...
		onOk : function()
		{
			...
			switch ( data.type || 'qwiki' )
			{
				...
				case 'qwiki':
					switch ( data.qwiki.type || 'topicurl')
					{
						...
						case 'attachmenturl':
							var protocol = '%PUBURL%/',
								url = ( data.qwiki.topicurl && data.qwiki.topicurl ) || '',
								urlPrefix = FoswikiCKE.getWeb()+'/'+FoswikiCKE.getTopic()+'/';
							if (0 === url.indexOf(urlPrefix))
								url = url.replace(urlPrefix, '');
							if (!url.match(/\//)) protocol = '%ATTACHURL%/';
							attributes['data-cke-saved-href'] = ( url.indexOf( '/' ) === 0 ) ? url : protocol + url;
							break;
					}
				...
				case 'anchor':
					var name = ( data.anchor && data.anchor.name ),
						id = ( data.anchor && data.anchor.id );
					url = attributes[ 'data-cke-saved-href' ] = '#' + ( name || id || '' );
					break;
				case 'email':
			...
			if ( !this._.selectedElement )
			{
				...
				if ( ranges.length == 1 && ranges[0].collapsed )
				{
					...
					var text = new CKEDITOR.dom.text( data.type == 'email' ?
							data.email.address : url, editor.document );
		...
		onFocus : function()
		{
			var linkType = this.getContentElement( 'info', 'linkType' ),
					urlField;
			if ( linkType && linkType.getValue( ) == 'qwiki' )
			{
...
//Advanced und Target ausschalten!
